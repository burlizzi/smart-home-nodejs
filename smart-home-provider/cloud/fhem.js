...
function FHEM(log, config, api) {
    ...
    if (config.ssl) {
        if (typeof config.ssl !== 'boolean') {
            //console.log('config: value for ssl has to be boolean.');
            process.exit(0);
        }
        base_url = 'https://';
    }
...
FHEM_reading2homekit(mapping, orig) {
    ...
    if (typeof mapping.reading2homekit === 'function') {
        ...
        try {
            ...
            console.log(mapping.informId + ' reading2homekit: ' + err);
            return undefined;
        }
        if (typeof value === 'number' && isNaN(value)) {
            console.log(mapping.informId + ' not a number: ' + orig);
            return undefined;
        }
    ...
    console.log('    caching: ' + (mapping.name ? 'Custom ' + mapping.name : mapping.characteristic_type) + (mapping.subtype ? ':' + mapping.subtype : '') + ': '
...
FHEM_reading2homekit_(mapping, orig) {
    ...
    if (reading == 'temperature'
        ...
        if (mapping.event_map !== undefined) {
            var mapped = mapping.event_map[value];
            if (mapped !== undefined) {
                console.log(mapping.informId + ' eventMap: value ' + value + ' mapped to: ' + mapped);
                value = mapped;
            }
        }
        ...
        if (value !== undefined && mapping.part !== undefined) {
            ...
            if (mapped === undefined) {
                console.log(mapping.informId + ' value ' + value + ' has no part ' + mapping.part);
                return value;
            }
            console.log(mapping.informId + ' parts: using part ' + mapping.part + ' of: ' + value + ' results in: ' + mapped);
            value = mapped;
        }
        ...
        if (mapping.threshold) {
            ...
            if (parseFloat(value) > mapping.threshold)
                ...
                mapped = 0;
            console.log(mapping.informId + ' threshold: value ' + value + ' mapped to ' + mapped);
            value = mapped;
        }
        ...
        if (typeof mapping.value2homekit_re === 'object' || typeof mapping.value2homekit === 'object') {
            ...
            console.log(mapping.informId + ' values: value ' + value + ' mapped to ' + mapped);
            value = mapped;
        }

        if (!format) {
            console.log(mapping.informId + ' empty format');
            ...
            if (mapped !== undefined) {
                console.log(mapping.informId + ' valueOn/valueOff: value ' + value + ' mapped to ' + mapped);
                value = mapped;
            }

            if (mapping.factor) {
                console.log(mapping.informId + ' factor: value ' + value + ' mapped to ' + value * mapping.factor);
            ...
            if (typeof mapped !== 'number') {
                console.log(mapping.informId + ' is not a number: ' + value);
                return undefined;
            }
            value = mapped;
            ...
            if (typeof mapped !== 'number') {
                console.log(mapping.informId + ' not a number: ' + value);
                return undefined;
            }
            value = mapped;
        ...
        if (mapping.max && mapping.maxValue) {
            ...
            console.log(mapping.informId + ' value ' + orig + ' scaled to: ' + value);
        }

        if (mapping.minValue !== undefined && value < mapping.minValue) {
            console.log(mapping.informId + ' value ' + value + ' clipped to minValue: ' + mapping.minValue);
            value = mapping.minValue;
        } else if (mapping.maxValue !== undefined && value > mapping.maxValue) {
            console.log(mapping.informId + ' value ' + value + ' clipped to maxValue: ' + mapping.maxValue);
            value = mapping.maxValue;
        }
        ...
        if (typeof value === 'number') {
            ...
            if (value !== mapped)
                console.log(mapping.informId + ' value: ' + value + ' inverted to ' + mapped);
            value = mapped;
        }
...
//console.log( 'value: ' + value );
            ...
            console.log('  error in longpoll connection: ' + err);
...
FHEM.prototype.connect = function (callback, filter) {
    ...
    console.log('Fetching FHEM devices...');
    ...
    this.connection.request.get({url: url, json: true, gzip: true},
        function (err, response, json) {
            if (!err && response.statusCode === 200 && json) {
                ...
                console.log('got: ' + json['totalResultsReturned'] + ' results');
                if (json['totalResultsReturned']) {
                    ...
                    sArray.map(function (s) {
                        ...
                        if (device) {
                            ...
                            console.log('no device created for ' + s.Internals.NAME + ' (' + s.Internals.TYPE + ')');
                            return undefined;
                        }
                    ...
                    this.devices.forEach(function (d) {
                        if (d.room) {
                            ...
                            console.log('Please add ' + d.name + ' to a room in FHEM.');
                        }
                    }.bind(this));
                ...
                console.log('There was a problem connecting to FHEM');
                if (response)
...
FHEMDevice.prototype.fromHomebridgeMapping = function (homebridgeMapping) {
    if (!homebridgeMapping)
        ...
        console.log('homebridgeMapping: ' + homebridgeMapping);

    if (homebridgeMapping.match(/^{.*}$/)) {
        try {
            homebridgeMapping = JSON.parse(homebridgeMapping);
        } catch (err) {
            console.log('  fromHomebridgeMapping JSON.parse: ' + err);
            return;
        }
    ...
    for (var mapping of homebridgeMapping.split(/ |\n/)) {
        ...
        if (match === undefined || match.length < 4 || !match[3]) {
            console.log('  wrong syntax: ' + mapping);
            continue;
        }
        ...
        for (var param of params.split(',')) {
            ...
            if (p.length == 2)
                ...
                console.log('  wrong syntax: ' + param);
...
FHEMDevice.prototype.prepare = function (mapping) {
    ...
    if (mapping.default !== undefined) {
        ...
        console.log('default: ' + mapping.default);
    }
    ...
    if (typeof mapping.values === 'object') {
        ...
        for (var entry of mapping.values) {
            ...
            if (!match) {
                console.log('values: format wrong for ' + entry);
                continue;
            }
    ...
    if (typeof mapping.cmds === 'object') {
        ...
        for (var entry of mapping.cmds) {
            var match = entry.match('^([^:]*)(:(.*))?$');
            if (!match) {
                console.log('cmds: format wrong for ' + entry);
                continue;
            }
    ...
    if (mapping.reading2homekit !== undefined && typeof mapping.reading2homekit !== 'function') {
        if (mapping.reading2homekit.match(/^{.*}$/)) {
            try {
                ...
                console.log('  reading2homekit: ' + err);
            ...
            if (typeof this.jsFunctions[mapping.reading2homekit] === 'function')
                ...
                console.log('  reading2homekit: no function named ' + mapping.reading2homekit + ' in ' + util.inspect(this.jsFunctions));
        }

        if (mapping.reading2homekit !== undefined && typeof mapping.reading2homekit !== 'function') {
            console.log('  reading2homekit disabled.');
            delete mapping.reading2homekit;
        }
    }
    ...
    if (mapping.homekit2reading !== undefined && typeof mapping.homekit2reading !== 'function') {
        if (mapping.homekit2reading.match(/^{.*}$/)) {
            try {
                ...
                console.log('  homekit2reading: ' + err);
            ...
            if (typeof this.jsFunctions[mapping.homekit2reading] === 'function')
                ...
                console.log('  homekit2reading: no function named ' + mapping.homekit2reading + ' in ' + util.inspect(this.jsFunctions));
        }

        if (mapping.homekit2reading !== undefined && typeof mapping.homekit2reading !== 'function') {
            console.log('  homekit2reading disabled.');
            delete mapping.homekit2reading;
        }
    }
};
...
FHEM.prototype.checkAndSetGenericDeviceType = function () {
    console.log('Checking devices and attributes...');
    ...
    this.execute(cmd,
        function (result) {
            ...
            if (!result.match(/(^| )homebridgeMapping\b/)) {
                ...
                console.log('homebridgeMapping attribute created.');
            }
            
            if (!result.match(/(^| )realRoom\b/)) {
                this.execute('{ addToAttrList( "realRoom:textField" ) }');
                console.log('realRoom attribute created.');
            }
            ...
            if (!result.match(/(^| )genericDeviceType:security,ignore,switch,outlet,light,blind,thermometer,thermostat,contact,garage,window,lock,aircondition,airpurifier,camera,coffeemaker,dishwasher,dryer,fan,kettle,oven,refrigerator,scene,sprinkler,vacuum,washer\b/)) {
                ...
                this.execute(cmd,
                    function (result) {
                        console.log('genericDeviceType attribute was not known. please restart.');
                        process.exit(0);
                    }.bind(this));
            }
...
FHEM_ct2rgb(ct) {
    ...
    if (temp <= 66)
        g = 99.4708025861 * console.log(temp) - 161.1195681661;
    else
    ...
    if (temp <= 19)
        b = 0;
    if (temp < 66)
        b = 138.5177312231 * console.log(temp - 10) - 305.0447927307;
    if (b < 0)
        b = 0;
    if (b > 255)
        b = 255;
...
FHEMDevice.prototype.query = function (mapping, callback) {
    ...
    console.log('query: ' + mapping.characteristic_type + ' for ' + mapping.informId);
    ...
    if (typeof mapping === 'object' && value !== undefined) {
        ...
        console.log('  cached: ' + value + ' (as ' + typeof(value) + (defined ? '; means ' + defined : '') + ')');
        ...
        /*console.log('not cached; query: ' + mapping.informId);
...
FHEMDevice.prototype.command = function (mapping, value, traitCommand) {
    ...
    console.log(this.name + ' sending command ' + c + ' with value ' + value);
    ...
    if (c == 'identify') {
        ...
        if (mapping.characteristic_type == 'On' && value) {
            if (this.delayed_timers && this.delayed_timers.length) {
                console.log(this.name + ': skipping set cmd for ' + mapping.characteristic_type + ' with value ' + value);
                return;
            }
        }

        console.log(this.name + ': executing set cmd for ' + mapping.characteristic_type + ' with value ' + value);

        if (typeof mapping.homekit2reading === 'function') {
            try {
                value = mapping.homekit2reading(value);
            } catch (err) {
                console.log(mapping.informId + ' homekit2reading: ' + err);
                return;
            }
            if (value === undefined) {
                console.log('  converted value is unchanged ');
                return;

            }

            console.log('  value converted to ' + value);
            ...
            if (typeof value === 'number') {
                ...
                if (value !== mapped) {
                    console.log('  value: ' + value + ' inverted to ' + mapped);
                    value = mapped;
                }

                if (mapping.factor) {
                    mapped /= mapping.factor;
                    console.log('  value: ' + value + ' mapped to ' + mapped);
                    value = mapped;
                }
        ...
        if (cmd === undefined) {
            console.log(this.name + ' no cmd for ' + c + ', value ' + value);
            return;
        }
    ...
    if (command === undefined) {
        console.log(this.name + ' Unhandled command! cmd=' + c + ', value ' + value);
        return;
    }
...
FHEMDevice(platform, s) {
    ...
    if (FHEM_isPublished(s.Internals.NAME)) {
        console.log(s.Internals.NAME + ' is already published');
        return;
    }

    if (!s.Readings) {
        console.log('ignoring ' + s.Internals.NAME + ' (' + s.Internals.TYPE + ') without readings');
        return;
    }
    if (!s.Attributes) {
        console.log('ignoring ' + s.Internals.NAME + ' (' + s.Internals.TYPE + ') without attributes');
        return;
    }

    if (s.Attributes.disable == 1) {
        console.log(s.Internals.NAME + ' is currently disabled');
    }
    ...
    if (genericType === 'ignore') {
        console.log('ignoring ' + s.Internals.NAME + ', genericDeviceType = ignore used');
        return;
    }
	
    if (s.Internals.TYPE === 'structure' && genericType === undefined) {
        console.log('ignoring ' + s.Internals.NAME + ' (' + s.Internals.TYPE + ') without genericDeviceType');
        return;
    }
    ...
    if (match = s.PossibleSets.match(/(^| )dim:slider,0,1,99/)) {
        ...
        // Hue
        console.log('detected HUEDevice');
        max = 100;
    ...
    if (s.Internals.TYPE === 'MilightDevice'
        ...
        // MilightDevice
        console.log('detected MilightDevice');
        ...
        // WifiLight
        console.log('detected WifiLight');
    ...
    console.log('mappings for ' + s.Internals.NAME + ': ' + util.inspect(this.mappings));

    if (this.service_name !== undefined) {
        console.log(s.Internals.NAME + ' is ' + this.service_name);
    } else if (this.mappings.CurrentPosition)
    console.log(s.Internals.NAME + ' is blind [' + this.mappings.CurrentPosition.reading + ']');
    else if (this.mappings.TargetTemperature)
    console.log(s.Internals.NAME + ' is thermostat [' + this.mappings.TargetTemperature.reading
    ...
    console.log(s.Internals.NAME + ' is contact sensor [' + this.mappings.ContactSensor.reading + ']');
    else if (this.mappings.OccupancyDetected)
        console.log(s.Internals.NAME + ' is occupancy sensor');
    else if (!this.mappings) {
        console.log(s.Internals.NAME + ': no service type detected');
        return;
    }
    ...
    if (this.service_name === 'lock' || this.service_name === 'garage' || this.service_name === 'window')
        ...
        console.log(s.Internals.NAME + ' has');
    for (characteristic_type in this.mappings) {
        ...
        for (mapping of mappings) {
            ...
            console.log('  ' + characteristic_type + ' [' + (mapping.device ? mapping.device + '.' : '') + mapping.reading + ';' + mapping.cmdOn + ',' + mapping.cmdOff + ']');
            else if (characteristic_type === 'Hue' || characteristic_type === 'Saturation')
            console.log('  ' + characteristic_type + ' [' + (mapping.device ? mapping.device + '.' : '') + mapping.reading + ';' + mapping.cmd + ';0-' + mapping.max + ']');
            else if (mapping.name) {
                if (characteristic_type === 'Volume')
                console.log('  Custom ' + mapping.name + ' [' + (mapping.device ? mapping.device + '.' : '') + mapping.reading + ';' + (mapping.nocache ? 'not cached' : 'cached' ) + ']');
                else
                console.log('  Custom ' + mapping.name + ' [' + (mapping.device ? mapping.device + '.' : '') + mapping.reading + ']');
            } else
            console.log('  ' + characteristic_type + ' [' + (mapping.device ? mapping.device + '.' : '') + mapping.reading + ']');
        }
    }

    if (this.mappings.reachable)
    console.log(s.Internals.NAME + ' has reachability [' + this.mappings.reachable.reading + ']');

//console.log( util.inspect(s) );

    // device info
    this.id = s.Internals.NAME;
        ...
        for (mappingRoot in mappings) {
                  ...
                  delete mapping.format;
        				...
        				if (s.Attributes.eventMap) {
        					...
        					if (mapping.event_map && Object.keys(mapping.event_map).length) console.log('event_map: ' + mapping.event_map);
        				}
